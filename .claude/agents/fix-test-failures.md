...
Critters is a single Cargo workspace (`critters-core`, `critters-native`, `critters-wasm`). Run `cargo` from the repo root; read `CLAUDE.md` for the build/test commands. The game logic under test is a port of `reference/game.js` — when a test encodes behaviour, the JavaScript is the ground truth.
