...
- Follow the conventions in this repo's CLAUDE.md (file header comments, the 800-line file limit enforced by `file_line_count`, test-first bug fixing, Rust naming, agg-gui-only rendering, exact behavioural match with `reference/game.js`). Run `cargo` commands from the repo root.
