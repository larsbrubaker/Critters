...
**These tests validate exact behavioral matching with the JavaScript original. There are no workarounds.**

Every test exists because it validates behaviour that must match the original exactly. Bypassing tests means shipping incorrect implementations.
...
4. Cross-reference with the JavaScript original to verify expected behavior
...
- Does the Rust implementation diverge from the JavaScript behaviour?
...
- **Algorithm errors**: Verify against the JavaScript step-by-step
...
3. Compare intermediate values with the JavaScript original (run it in a browser with `?debug` and `window.__cs`)
...
## JavaScript Comparison Technique

When stuck, compare execution step-by-step with the JavaScript original:

1. Add corresponding print statements in both Rust and the JavaScript
